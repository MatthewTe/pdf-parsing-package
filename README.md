...
Where `Name` is the name of a table in the database and `Date` and `Pdf_type` are the date and pdf category that are associated with the pdf of said table.
...
#### `cosine_sim(str_1, str_2)`
This is largely a decorative method and is used to calculate the cosine similarity between two input strings. This method follows the method of calculating cosine similarity from the sklearn package described in the following [article](https://www.machinelearningplus.com/nlp/cosine-similarity/). The method merely wraps that process in a manner that makes it compatible with the rest of the database api.

Example:
```python
str_1 = """Method that returns a tuple of a pdf table name and the pdf table name from
the previous year given an input tuple and a list of tuples returned from ticker_tables"""

str_2 = """Method that computes the cosine similarity between two input strings"""

pdf_db.cosine_sim(str_1, str_2)
